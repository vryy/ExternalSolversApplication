...
public:
    ...
    {
        if(mVerbosity > 0) std::cout << "AMGCL Linear Solver, Setting up AMGCL for iterative solve " << std::endl;
        ...
        ) override
    {
        ...
        if(mUseAMGPreconditioning && mProvideCoordinates && (mBlockSize == 2 || mBlockSize == 3)) {
            ...
            if (static_block_size != 1 && static_block_size != 3) {
                std::cout << "AMGCL Linear Solver can only combine block matrices with coordinates in 3D. Falling back to scalar matrices" << std::endl;
                static_block_size = 1;
            }
        ...
        if (mTolerance < resid) std::cout << "AMGCL Linear Solver: Non converged linear solution. ["<< resid << " > "<< mTolerance << "]" << std::endl;

        if(mVerbosity > 1)
            std::cout << "AMGCL Linear Solver:" << std::endl
        ...
        ) override
    {
        ...
        else //distribute
        {
            ...
            int max_block_size = rModelPart.GetCommunicator().MaxAll(mBlockSize);
        ...
        if (mVerbosity > 1) std::cout << "AMGCL Linear Solver: mndof: " << mBlockSize << std::endl;
